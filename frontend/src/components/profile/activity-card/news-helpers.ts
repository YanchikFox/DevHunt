...
function extractImagesFromAttachments(
    attachments?: Array<{ type: string; url?: string; downloadUrl?: string; contentType?: string; fileName?: string }>
): string[] {
...
export function extractNewsContent(activity: UserActivityFeedItem): NewsContent | null {
    ...
    if (activity.newsContent || activity.newsTitle) {
        ...
        const payloadImages = payload ? getPayloadValue(payload, "images", "Images", [] as string[]) : []
